...
if [ "$OUTPUT_JSON" = true ]; then
    ...
    if [ "$RUN_BACKEND" = true ]; then
        run_json_check "backend-lint" \
            "uv run ruff check app ../scripts/local_state_archive.py tests/core/test_logging.py tests/core/test_runtime_security_defaults.py tests/core/test_txt2crs_settings.py tests/scripts/test_container_contract.py tests/scripts/test_generate_client_contract.py tests/scripts/test_local_backup_contract.py tests/scripts/test_port_allocation_contract.py tests/scripts/test_quality_workflow_contract.py tests/scripts/test_security_workflow_contract.py tests/scripts/test_start_local_script.py" \
        ...
        run_json_check "backend-baseline-tests" \
            "uv run pytest --confcutdir=tests/core tests/core/test_logging.py tests/core/test_runtime_security_defaults.py tests/core/test_txt2crs_settings.py -q && uv run pytest --confcutdir=tests/scripts tests/scripts/test_container_contract.py tests/scripts/test_generate_client_contract.py tests/scripts/test_local_backup_contract.py tests/scripts/test_port_allocation_contract.py tests/scripts/test_quality_workflow_contract.py tests/scripts/test_security_workflow_contract.py tests/scripts/test_start_local_script.py -q" \
            "$PROJECT_ROOT/backend"
    fi
