...
subscription. From the repository root, bootstrap the dedicated identity with
the short helper; no installed Codex CLI is required:

```bash
./scripts/auth-codex.sh
```

Then run the live test from `backend/` against the resulting app-owned
