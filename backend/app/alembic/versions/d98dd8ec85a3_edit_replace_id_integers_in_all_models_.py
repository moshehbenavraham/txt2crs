...
def downgrade():
    raise CommandError(
        "Downgrade blocked for revision d98dd8ec85a3: the original UUID->integer "
        "rollback path reassigns new integer IDs and is not identity-preserving. "
        "Use backup restore for rollback, or implement an explicit UUID<->integer "
        "mapping migration before enabling downgrade."
    )
