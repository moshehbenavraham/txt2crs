...
def _assert_null_email_validation_problem(response) -> None:
    ...
    assert payload["type"].endswith(ErrorCode.VALIDATION_ERROR.value)
    assert any(
        "cannot be null" in error.get("message", "").lower()
...
def test_create_user_new_email(
    ...
    with (
        patch(
            "app.api.routes.users.send_email_with_retry", return_value=None
        ) as send_email_mock,
        ...
        email_payload = send_email_mock.call_args.kwargs["html_content"]
        assert "/reset-password?token=" in email_payload
        assert password not in email_payload
...
def test_get_non_existing_user_as_superuser(
    ...
    payload = response.json()
    assert payload["code"] == ErrorCode.USER_NOT_FOUND.value
...
def test_retrieve_users_orders_newest_first(
    client: TestClient, superuser_token_headers: dict[str, str], db: Session
) -> None:
    older_user = crud.create_user(
        session=db,
        user_create=UserCreate(email=random_email(), password=random_lower_string()),
    )
    newer_user = crud.create_user(
        session=db,
        user_create=UserCreate(email=random_email(), password=random_lower_string()),
    )
    older_user.created_at = datetime(2026, 1, 1, tzinfo=UTC)
    newer_user.created_at = older_user.created_at + timedelta(days=1)
    db.add(older_user)
    db.add(newer_user)
    db.commit()
    ...
    assert ids.index(str(newer_user.id)) < ids.index(str(older_user.id))
    assert next(user for user in data if user["id"] == str(newer_user.id))["created_at"]
...
def test_update_user_me_rejects_explicit_null_email(
    client: TestClient, normal_user_token_headers: dict[str, str]
) -> None:
    ...
    _assert_null_email_validation_problem(response)
...
def test_update_user_rejects_explicit_null_email(
    client: TestClient, superuser_token_headers: dict[str, str], db: Session
) -> None:
    username = random_email()
    password = random_lower_string()
