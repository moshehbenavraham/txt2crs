...
check_package() {
    ...
    if [[ -n "$pkg_entry" && "$pkg_entry" != "null" ]]; then
        local stack=""
        stack=$(echo "$pkg_entry" | jq -r '.stack_hint // .stack // empty' 2>/dev/null) || true
        if [[ -n "$stack" ]]; then
            if [[ "$OUTPUT_MODE" == "human" ]]; then
...
check_workspace_tools() {
    ...
    local uv_workspace_manifest=""

    # This repository keeps its uv workspace manifest under the registered
    # backend package instead of the repository root. Inspect registered
    # package paths so the prerequisite check understands that valid mixed
    # Python/npm monorepo layout.
    while IFS= read -r package_path; do
        local package_manifest="${package_path}/pyproject.toml"
        if [[ -f "$package_manifest" ]] \
            && grep -q '^\[tool\.uv\.workspace\]$' "$package_manifest"; then
            uv_workspace_manifest="$package_manifest"
            break
        fi
    done < <(get_packages | jq -r '.[].path')
    ...
    elif [[ -n "$uv_workspace_manifest" ]]; then
        manager="uv"
        manager_version=$(uv --version 2>/dev/null | awk '{print $2}' || echo "not installed")
    fi
