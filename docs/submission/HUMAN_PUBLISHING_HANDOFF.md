...
git tag --annotate v1.0.0 \
  --message "txt2crs 1.0.0 - OpenAI Build Week Education release" \
  "${final_commit}"
...
Verify that remote `main` and peeled `v1.0.0` resolve to `final_commit`. Make
...
- remote `main` and `v1.0.0` match the reviewed release commit;
