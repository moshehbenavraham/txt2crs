...
- intended repository release: `v1.0.2`.
...
git tag --annotate v1.0.2 \
  --message "txt2crs 1.0.2 - OpenAI Build Week Education release" \
  "${final_commit}"
...
Verify that remote `main` and peeled `v1.0.2` resolve to `final_commit`. Make
...
- remote `main` and `v1.0.2` match the reviewed release commit;
