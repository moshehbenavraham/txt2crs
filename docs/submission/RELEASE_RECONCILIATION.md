...
3. the final commit that the human operator will tag as `v1.0.3`.
...
3. creates annotated tag `v1.0.3` on that exact commit;
...
Any tracked change after `v1.0.3` requires a new SemVer release rather than
moving the tag.
